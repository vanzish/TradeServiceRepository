...
namespace TradeService
{
    public class StructHelper
    {
        ...
        public void Open(FileAccess FileAccess, FileShare FileShare)
        {
            if(FileHelper.TryOpen(_file, 3))
            {
                LoadFileStream(FileMode.Open, FileAccess, FileShare);
            }
            else
            {
                throw new FileNotFoundException(string.Format("File {0} was not found or not accessible", _file));
            }
        }

        public void Create(FileAccess FileAccess, FileShare FileShare)
        {
            LoadFileStream(FileMode.Create, FileAccess, FileShare);
        }
    }
